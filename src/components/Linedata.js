...
import {Line} from 'react-chartjs-2';
  ...
  componentDidMount(){
    axios.get("https://api.covid19india.org/data.json").then((res) => {
      ...
      res.data.cases_time_series.map(val=>{con.push(val.totalconfirmed)})
      res.data.cases_time_series.map(val=>{rec.push(val.totalrecovered)})
      res.data.cases_time_series.map(val=>{dea.push(val.totaldeceased)})
       this.setState({
    ...
    <div>
      ...
      <div class="card mt-5 mb-2 ml-3 mr-3 shadow-lg border border-dark mb-5 bg-light rounded">
        <div class="card-header bg-primary text-light font-weight-bold">
           COVID Total Confirmed vs Recovered vs Death Graph (INDIA)
        </div>
